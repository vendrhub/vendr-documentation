---
title: ReadOnlyTranslatedValue<T>
description: API reference for ReadOnlyTranslatedValue<T> in Vendr, the eCommerce solution for Umbraco v8+
---
## ReadOnlyTranslatedValue&lt;T&gt;

```csharp
public class ReadOnlyTranslatedValue<T> : ValueObjectBase
```

**Inheritance**

* class [ValueObjectBase](../valueobjectbase/)

**Namespace**
* [Vendr.Core.Models](../)

### Indexers

#### ReadOnlyTranslatedValue&lt;T&gt;

```csharp
public T this[string languageIsoCode] { get; }
```


### Methods

#### DeepClone

```csharp
public override object DeepClone()
```


---

#### GetDefaultValue

```csharp
public T GetDefaultValue()
```


---

#### GetValue

```csharp
public T GetValue(string languageIsoCode, bool fallbackToDefault = true)
```


---

#### HasValue

```csharp
public bool HasValue(string languageIsoCode)
```


---

#### ToString

```csharp
public override string ToString()
```


---

#### TryGetValue

```csharp
public bool TryGetValue(string languageIsoCode, out T value)
```


### Operators

#### ReadOnlyTranslatedValue&lt;T&gt; Implicit

```csharp
public static implicit operator T(ReadOnlyTranslatedValue value)
```


<!-- DO NOT EDIT: generated by xmldocmd for Vendr.Core.dll -->
